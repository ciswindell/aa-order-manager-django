...
class WorkflowExecutor:
    ...
    def execute_workflow(
        self, workflow: WorkflowBase, input_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not is_valid:
                self.logger.warning("Workflow validation failed: %s", validation_error)
            ...
            self.logger.info("Executing workflow: %s", workflow_id)
            ...
            self.logger.info(
                "Workflow completed successfully: %s (%.2fs)",
                workflow_id,
                execution_time,
            )
            ...
            self.logger.error(
                "Workflow execution failed: %s", workflow_id, exc_info=True
            )

            # Use workflow's error handling
            error_result = workflow.handle_errors(
                error, {"input_data": input_data, "execution_time": execution_time}
            )
    ...
    def register_workflow_type(self, workflow_type: str, workflow_class: type) -> None:
        ...
        self.logger.info("Registered workflow type: %s", workflow_type)

    def create_workflow(
        self, workflow_type: str, config: Optional[Dict[str, Any]] = None
    ) -> WorkflowBase:
        """
        ...
        self.logger.debug("Created workflow instance: %s", workflow_type)
        return workflow
    ...
    def unregister_workflow_type(self, workflow_type: str) -> bool:
        ...
        if workflow_type in self._workflow_registry:
            ...
            self.logger.info("Unregistered workflow type: %s", workflow_type)
            return True
        return False
