...
class CloudServiceFactory:
    ...
    @classmethod
    def create_service(cls, provider: str = None) -> CloudOperations:
        """
        ...
        try:
            if provider.lower() == "dropbox":
                return cls._create_dropbox_service()
            else:
                raise CloudServiceError(f"Unsupported cloud provider: {provider}", provider)
                
        except Exception as e:
            logger.error(f"Failed to create cloud service for provider '{provider}': {e}")
            raise CloudServiceError(f"Service creation failed: {str(e)}", provider)
    
    @classmethod
    def _create_dropbox_service(cls) -> CloudOperations:
        """Create Dropbox cloud service instance."""
        from ..dropbox.cloud_service import DropboxCloudService
        from ..dropbox.auth import create_dropbox_auth
        
        # Create auth service using config
        auth_service = create_dropbox_auth()
        
        # Create and return cloud service
        return DropboxCloudService(auth_type=config.get_dropbox_auth_type())
...
def get_cloud_service(provider: str = None) -> CloudOperations:
    """
    ...
    """
    return CloudServiceFactory.create_service(provider) 