...
def format_report_description(report: "Report") -> str:
    """
    Format report legal description with date range (plain text).
...
def format_report_description_html(report: "Report") -> str:
    ...
    Returns:
        HTML-formatted string with legal description and date range
    """
    legal_desc = report.legal_description or "[No description]"
    ...
    if report.start_date and report.end_date:
        start_str = _format_date(report.start_date)
        end_str = _format_date(report.end_date)
        date_range = f" from <strong>{start_str}</strong> to <strong>{end_str}</strong>"
        return f"{legal_desc}{date_range}"

    # Only start date
    if report.start_date and not report.end_date:
        start_str = _format_date(report.start_date)
        return f"{legal_desc} from <strong>{start_str}</strong> to <strong>present</strong>"

    # Only end date
    if not report.start_date and report.end_date:
        end_str = _format_date(report.end_date)
        return f"{legal_desc} from <strong>inception</strong> to <strong>{end_str}</strong>"
