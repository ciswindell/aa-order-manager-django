"""
Document Archive Finder Service.

This service searches for existing document archives in cloud storage
...
from orders.services.lease.runsheet_results import ArchiveSearchResult
...
class DocumentArchiveFinder:
    """
    Service to find existing document archives in cloud storage.

    This service checks if a document archive directory exists for a lease,
    ...
    def __init__(self, repository: DocumentImagesLinkRepository = None):
        """
        Initialize DocumentArchiveFinder.
    ...
    def find_archive(
        ...
        """
        Search for an existing document archive in cloud storage.
        ...
        logger.info("Searching for document archive at: %s", directory_path)

        try:
            files = cloud_service.list_files(directory_path)

            if not files:
                logger.info("Document archive not found at: %s", directory_path)
            ...
            logger.info("Document archive found, creating share link")
            ...
            logger.info(
                "Successfully %s share link for document archive. Location ID: %s",
                action,
                cloud_location.id,
            )
